...
from concurrent.futures import ThreadPoolExecutor, ProcessPoolExecutor, Executor, wait

###############################################################################
#
# Tesseract Python Wrapper - pytesseract Modified
#
###############################################################################

class IPytesseract(object):
...
class Image_OCR(object):
    ...
    def IOCR(self, Group_Coordinates_i) :
        
        group_coordinates_i = Group_Coordinates_i
        ...
        for i in range(len(group_coordinates_i)):  
             
            x1m = group_coordinates_i[i][0]
            y1m = group_coordinates_i[i][1]
            x2m = group_coordinates_i[i][2]
            y2m = group_coordinates_i[i][3]
            ...
            cv2.imwrite('icon\\'+'s'+OCR_string_processed+'.png',crop_img)
            ...
            for k in range(len(target_keyword_i)):
                ...
                if (Levenshtein.ratio(OCR_string_processed, target_keyword_i[k]))>0.8:               
                    ...
                    medical_positive_coordinates[k] = group_coordinates_i[i]
        ...
        if len(operation_coordinates) >0:
    ...
    def ISearch(self, Multi_Threads = None):
        ...
        with ProcessPoolExecutor(max_workers=multi_threads) as pool:
        
            for group_coordinates_i in group_coordinates:
            
                future = pool.submit(self.IOCR, group_coordinates_i)
                
                futures.append(future)
            
            for future in futures:
                    
                operation_coordinates.append(future.result())
                
        
        print(operation_coordinates)
